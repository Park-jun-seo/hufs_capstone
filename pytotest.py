...
def va_mfcc(s):
    ...
    MFCCs = librosa.feature.mfcc(y, sr, n_fft=input_nfft , hop_length=input_stride , n_mfcc=64)
...
ch = {'시':0,'청':3,'의':6,'창':1,'살':9,'은':5,'외':2,'창':10,'살':8,'이':4,'다':7,'*':11,'-':12}
num = {0:'시',3:'청',6:'의',1:'창',9:'살',5:'은',2:'외',10:'창',8:'살',4:'이',7:'다',11:'*',12:'-'}
...
net1 = GRU_seq(13,64,64,1).to(device) 
...
optimizer = torch.optim.Adam(net1.parameters(), lr=learning_rate)
  ...
  ###############################모델 불러오기#######################################
checkpoint = torch.load('G:\\내 드라이브\\캡스톤프로젝트\\ver.3\\model\\first_para_1000.tar')
...
net1.eval()
print(epoc_h)
ur = "G:\\내 드라이브\\캡스톤프로젝트\\ver.3\\음성\\B3.wav"
#ur = "G:\\내 드라이브\\캡스톤프로젝트\\ver.3\\정답음성\\an7.wav"
